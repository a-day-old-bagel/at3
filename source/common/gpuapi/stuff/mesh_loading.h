...
std::vector<at3::MeshAsset<EcsInterface>> loadMesh(const char* filepath, bool combineSubMeshes, at3::VkhContext& ctxt);

template <typename EcsInterface>
std::vector<at3::MeshAsset<EcsInterface>> loadMesh(const char* filepath, bool combineSubMeshes, at3::VkhContext& ctxt)
{
  using namespace at3;
  std::vector<MeshAsset<EcsInterface>> outMeshes;
  ...
  if (scene)
  {
    ...
    for (uint32_t mIdx = 0; mIdx < scene->mNumMeshes; mIdx++)
    {
      ...
      if (!combineSubMeshes)
      {
        at3::Mesh::make<EcsInterface>
            (outMeshes[mIdx], ctxt, vertexBuffer.data(), numVerts, indexBuffer.data(), indexBuffer.size());
      }
    }
    ...
    {
      at3::Mesh::make<EcsInterface>
          (outMeshes[0], ctxt, vertexBuffer.data(), numVerts, indexBuffer.data(), indexBuffer.size());
    }
  }
