...
namespace at3 {
  ...
  */
  ...
  /** @brief 2D texture */
  template <typename EcsInterface>
  struct VkcTexture2D : public VkcTexture<EcsInterface> {
  ...
  class VkcTextureRepository {
      std::vector<VkcTexture2D<EcsInterface>> textures;
      std::vector<VkDescriptorImageInfo> descriptorImageInfos;
      std::unordered_map<std::string, uint32_t> textureArrayIndexMap;
      uint32_t nextId = 0;
      void registerNewTexture2D(VkcTexture2D<EcsInterface>* texture, VkcTextureOperationInfo &info,
