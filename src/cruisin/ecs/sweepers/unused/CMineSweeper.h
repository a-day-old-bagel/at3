...
class CMinesweeper {
  ...
  public:
    ...
    bool Update(std::vector<SVector2D> &mines);

    //used to transform the sweepers vertices prior to rendering
    void WorldTransform(std::vector<SPoint> &sweeper);

    //returns a vector to the closest mine
    SVector2D GetClosestMine(std::vector<SVector2D> &objects);

    //checks to see if the minesweeper has 'collected' a mine
    int CheckForMine(std::vector<SVector2D> &mines, double size);
    ...
    void PutWeights(std::vector<double> &w) { m_ItsBrain.PutWeights(w); }
